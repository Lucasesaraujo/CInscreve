...
async function loginService(email, password, userAgent, ip) {
  try {
        ...
        $set: {
          ...
          ngo: {
            ...
            sustainable_development_goals:
              dadosUsuarioExterno.ngo.sustainable_development_goals,
          },
        },
      },
    ...
    return { usuario: usuarioLocal, accessToken, refreshToken };
  } catch (error) {
    logger.error(
      `Erro no serviço de login para ${email}:`,
...
async function getUserService(userPayload) {
  try {
    logger.info(
      `Buscando dados completos do usuário logado: ID ${userPayload.id}`
    );
...
async function logoutService(accessToken) {
  try {
    ...
    if (resultado.deletedCount === 0) {
      logger.warn(
        "Tentativa de logout de token não encontrado ou já removido."
      );
...
async function renewTokenService(refreshToken) {
  ...
  if (!tokenDB) {
    const error = new Error(
      "Refresh Token inválido ou expirado no banco de dados"
    );
    error.status = 403;
    throw error;
  }
  ...
  return new Promise((resolve, reject) => {
    jwt.verify(
      ...
      async (err, decoded) => {
        if (err) {
          logger.warn(
            "Refresh Token inválido ou expirado ao verificar JWT:",
            err.message
          );
        ...
        if (!usuario) {
          logger.warn(
            `Usuário com ID ${decoded.id} não encontrado para renovação de token.`
          );
          const error = new Error(
